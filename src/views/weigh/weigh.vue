...
<div class="cartPage weigh">
        ...
        <div class="left">
                  ...
                  <div id="testccc" style="background:#fff;width:340px;color:#494e52;padding-top:5px;display:none;">
                      <header style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;padding-top:5px;"><font size="5">{{printerData.proName}}</font></header>
                      <p style="line-height:14px;font-size:14px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;height:12px;"><font size="4">打印时间:</font><span style="margin-left:5px;"><font size="4">{{printerData.time}}</font></span></p>
                      <p style="line-height:14px;font-size:14px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;height:12px;"><font size="4">规格:</font><span style="margin-left:5px;"><font size="4">{{printerData.sku}}</font></span></p>
                      <div style="overflow:hidden;">
                          <div style="display: inline-block;line-height:14px;height:13px;height:12px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;width: 190px;"><font size="4">单价:</font><span style="margin-left:5px;"><font size="4">￥{{printerData.price}}</font></span></div>
                          <div style="float:right;line-height:14px;height:12px;margin-right:15px;"><font size="4">重量:</font><span class="way" style="margin-left:5px;font-size:14px;"><font size="4">{{printerData.weigh}}</font></span></div>
                      </div>
                      <p style="line-height:14px;font-size:14px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;height:12px;"><font size="4">总价:</font><span style="margin-left:5px;"><font size="4">￥{{printerData.total}}</font></span></p>
    ...
    bodyKeyUp(e){
       ...
       codeAddCart:function(dt,addnum){
                ...
                if((dt.storeStock-num-addnum)>=dt.safeStock){
                    if(num==0){
                        this.weighCart.push({proName:dt.productName,color:dt.specification1?dt.specification1:'',
                                        size:dt.specification2?dt.specification2:'',specification3:dt.vesion?dt.specification3:'',
                                        img:dt.showPic,saleprice:dt.salePrice,newprice:'',
                                        num:addnum,skuid:dt.autoId,proId:dt.productId,shopskuid:dt.shopSkuId,
                                        joinFixedPrice:dt.joinFixedPrice,
                                        isStock:true,safeStock:dt.safeStock,storeStock:dt.storeStock,limitBuy:dt.limitBuy,
                                        productSaleMethod:dt.productSaleMethod,measureUnit:!dt.measureUnit?"":dt.measureUnit,marketPrice:dt.marketPrice});
                    }else{
       ...
       skuchange:function(ev,id,type){
             ...
             switch(type){
                case 'color': var skuid=this.skuSelect.proid+"_"+id+"_"+this.skuSelect.size+"_"+this.skuSelect.version; break;
                case 'size': var skuid=this.skuSelect.proid+"_"+this.skuSelect.color+"_"+id+"_"+this.skuSelect.version; break;
                case 'version': var skuid=this.skuSelect.proid+"_"+this.skuSelect.color+"_"+this.skuSelect.size+"_"+id; break;
             }
       ...
       addskuPro:function(){
           if(this.checkedProduct.skus.length>1){
                if(this.color.length>0&&this.skuSelect.color=='0'){
                        this.$toast('请选'+this.color[0].name);return;
                }
                if(this.size.length>0&&this.skuSelect.size=='0'){
                        this.$toast('请选'+this.size[0].name);return;
                }
                if(this.version.length>0&&this.skuSelect.version=='0'){
                        this.$toast('请选'+this.version[0].name);return;
                }
           }
       ...
       addCart:function(idx,addnum,proId,isAddCart){
              ...
              addnum=addnum || 1;
              var num=0,j=0,joinFixedPriceNum=0;
              ...
              if((stock-num-addnum)>=safeStock){
                   if(num==0){
                        this.weighCart.push({proName:data.name,color:data.skus[idx].baseSKU.specification1,
                                        size:data.skus[idx].baseSKU.specification2,vesion:data.skus[idx].baseSKU.specification3,
                                        img:data.imageUrl,saleprice:data.skus[idx].salePrice,newprice:'',
                                        num:addnum,skuid:data.skus[idx].skuId,proId:this.skuSelect.proid,
                                        shopskuid:data.skus[idx].id,joinFixedPrice:data.joinFixedPrice,
                                        isStock:true,safeStock:data.safeStock,storeStock:stock,limitBuy:data.limitBuy,
                                        productSaleMethod:data.productSaleMethod,measureUnit:!data.measureUnit?"":data.measureUnit,marketPrice:data.marketPrice});
                   }else{
                        var a=this.weighCart[j].num,b=addnum;
                        this.weighCart[j].num=parseFloat(this.weighCart[j].num)+parseFloat(addnum);
                        if(this.weighCart[j].num.toString().split(".").length==2){
                            this.weighCart[j].num=this.weighCart[j].num.toFixed(3);
                        }
