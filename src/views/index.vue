    ...
    <!-- <div class="left">
      <div class="left_cont placeholder">
        <div class="memberInfo">
          <p v-if="member"><span>{{member.nick&&member.nick!=""?member.nick:member.userName}}</span><span>{{member.cellPhone || ''}}</span>
        ...
        <div class="cartList">
          <ul>
            <li v-for="(item,i) in cart" v-if="item.isStock" :key="item.skuid" @mouseover="cartHover(item.skuid)" @mouseout="cartHover(-1)" :class="{'active':item.skuid==checkedProid,'cartHover':item.skuid==cartHoverId}">
              ...
              <div class="li_right" v-if="!item.productSaleMethod">
                <i class="iconfont icon-shanjian" @click="upDataCartNum(item.skuid,-1)"></i>
                <input type="text" v-model="item.num" class="input2" @input="weighListInput(item.skuid,1)">
                <i class="iconfont icon-tianjia" @click="upDataCartNum(item.skuid,1)"></i>
              </div>
              <div class="li_right input" v-if="item.productSaleMethod">
                <input type="text" v-model="item.num" @input="weighListInput(item.skuid,2)">
              </div>
            </li>
            <li class="nostockCart" v-for="item in cart" :key="item.skuid" v-if="!item.isStock" @mouseover="cartHover(item.skuid)" @mouseout="cartHover(-1)" :class="{'active':item.skuid==checkedProid,'cartHover':item.skuid==cartHoverId}">
          ...
          <div class="clearAll" v-show="cart.length>0">
            <mu-raised-button label="清空列表" @click="cart=[]" fullWidth/>
          </div>
        </div>
        <div class="totalAll">
          <span>合计:</span><em style="float: right;margin-right: 20px;color: #212121;font-size: 16px;">{{totalAll().total | rmb}}</em>
        </div>
        <mu-raised-button class="shoukun" @click="toOrder" label="收款" fullWidth/>
      </div>
    </div>
    <div class="right">
      <div class="right_cont">
        <div class="qrPro">
          <mu-auto-complete class="aaa" style="width:100%;" id="" @keydown.enter="GetOncePor" v-model="productCode" id="ffocus" fullWidth hintText="请输入或扫描商品条形码" />
        </div>
        <div class="pro_menu" id="pro_menu">
          <ul :style="{'-webkit-transform':'translateX('+menu.menuOffsetL+'px)'}">
            <li v-for="item in typeList" :id="item.id" :key="item.id" @click="changeType(item.id)" :class="checkType==item.id?'active':''">
        ...
        <div class="proList" id="proList">
          <div class="gridlist-demo-container">
            <ul>
              <li v-for="item in productList" :key="item.id" @click="open(item.id,item.storeStock<=item.safeStock*item.skus.length)" class="imgList" :class="{nostock:item.storeStock<=item.safeStock*item.skus.length,fixedprice:item.joinFixedPrice}">
                <div>
        ...
        <div class="ar_cancelled container">
          <button type="button" @click="cancelled" class="cancelled_button" v-bind:class="{ Disable: buttonDisable[0] }">挂单{{singleOver}}</button>
          <button type="button" @click="getsingle" class="single_button" v-bind:class="{ Disable: buttonDisable[1] }">取单<i>{{singleNumber}}</i></button>
        </div>
      </div>
    </div> -->
    <div>
